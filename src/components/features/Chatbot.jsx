          ...
          >
            ...
            <div className="p-6 border-b border-white/5 flex justify-between items-center bg-quantum-deep/40 backdrop-blur-md">
              <div className="flex items-center gap-4">
                <div className={`p-2.5 bg-quantum-cyan/10 rounded-xl text-quantum-cyan border border-quantum-cyan/20 shadow-neon-sm ${loading ? 'animate-pulse' : ''}`}>
                  {loading ? (
                    <Lucide.RefreshCw size={20} className="animate-spin" />
                  ) : (
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <p className="text-[9px] text-quantum-purple font-black tracking-[0.2em] uppercase">
                      {loading ? 'Sincronizando...' : 'Módulo de Asistencia'}
                    </p>
                    {loading && <span className="w-1 h-1 bg-quantum-cyan rounded-full animate-ping" />}
                  </div>
                </div>
              </div>
              ...
              {msgs.length === 0 && (
                <div className="text-center py-20 opacity-30">
                  ...
                  <p className="text-[9px] font-black uppercase tracking-[0.4em]">
                    {loading ? 'Inicializando Enlace...' : 'Sincronización Lista'}
                  </p>
                </div>
              )}
              {msgs.map((m, i) => (
            ...
            <div className="p-5 border-t border-white/5 bg-quantum-deep/60">
              <div className="relative group">
                <input
                  ...
                  disabled={loading}
                  placeholder={loading ? "Sincronizando catálogo con Aura..." : "Inicia protocolo de consulta..."}
                  className={`w-full glass-quantum border-white/10 rounded-2xl pl-5 pr-14 py-4 text-sm text-white placeholder-quantum-cyan/20 focus:outline-none focus:border-quantum-cyan/30 focus:shadow-neon-sm transition-all ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                />
                <button
                  onClick={handleSend}
                  disabled={loading || !input.trim()}
                  className={`absolute right-3 top-1/2 -translate-y-1/2 p-2.5 text-quantum-cyan hover:text-white transition-all transform hover:scale-110 active:scale-95 ${loading || !input.trim() ? 'opacity-30 cursor-not-allowed' : ''}`}
                >
